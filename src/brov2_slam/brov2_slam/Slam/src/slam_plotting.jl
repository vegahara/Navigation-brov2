...
# import RoMEPlotting.plotSLAM2D
...
function plotSLAM2DPoses_mine( fg::AbstractDFG;
    ...
    Gadfly.push_theme(thm)

    if lbls
      psplt = Gadfly.plot(
        Gadfly.layer(x=Xpp,y=Ypp,label=LBLS,Geom.path(), Geom.label),
        Coord.cartesian(fixed=true), Guide.xlabel("East [m]"), Guide.ylabel("North [m]")
      )
    else
      psplt = Gadfly.plot(
    ...
    if drawContour
      for vsym in variableList
        pln = plotKDE(fg, vsym, title="title", solveKey=solveKey, dims=[1;2], levels=levels, c=[(manualColor === nothing ? "gray90" : manualColor);])
...
function plotSLAM2DLandmarks_mine( fg::AbstractDFG;
                              solveKey::Symbol=:default,
...
function plotSLAM2D_mine(fgl::AbstractDFG;
                    solveKey::Symbol=:default,
                    from::Int=0, to::Int=(2^(Sys.WORD_SIZE-1)-1), 
                    minnei::Int=0,
                    meanmax=:null,
